...
const main = async () => {
    ...
    let result = a.data.data.map(item => {
        return `
           ...
           <td>${item.amount}</td>
           <td><button data-id=${item.id} class="delete-btn">Delete</button><button class="edit-btn"><a data-id=${item.id} href="./edit-product/edit-product.html?id=${item.id}">Edit</a></button></td>
            `
    })
